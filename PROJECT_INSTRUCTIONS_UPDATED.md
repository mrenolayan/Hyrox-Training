...
`PHASE7_STATE.md`, `PHASE8_STATE.md`, `CLAUDE.md` (repo guide for Claude Code). Reference these
...
- Enforced by Supabase RLS — the database is the security boundary, not the
  ...
  rolled back in Phase 8 (completed).
...
- Never activate athlete/coach RLS while the relevant `user_id` could be
  null — lockout risk.
- **Production gate:** Phase 8 testing on `hyrox-slot-1` + `hyroxdev` must
  be complete and verified before production deployment to `rpm-athletics.vercel.app`
  + production Supabase.

## Current phase

**Phase 8 — Athlete Login & Invite (COMPLETE & VERIFIED)**
- Magic-link auth ✓
- Invite creation flow ✓
- Invite acceptance ✓
- RLS enforcement ✓
- End-to-end testing on preview environment ✓

**Next: Phase 8 Production Deployment**
- Deploy Phase 8 code to `rpm-athletics.vercel.app`
- Configure production Supabase auth
- Run Phase 8 migration against production
- Verify coach dashboard loads
- Plan legacy athlete migration and data cutover
